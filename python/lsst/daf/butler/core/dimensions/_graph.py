...
class DimensionGraph:
    ...
    def __new__(
        ...
        if conform:
            # Expand given dimensions to include all dependencies.  Keep
            # iterating until the set of names stops growing.
            # This is not as efficient as it could be, but we work pretty hard
            # cache DimensionGraph instances to keep actual construction rare,
            # so that shouldn't matter.
            oldSize = len(conformedNames)
            while True:
                # iterate over a temporary copy so we can modify the original
                for name in tuple(conformedNames):
                    conformedNames.update(universe[name].required.names)
                    conformedNames.update(universe[name].implied.names)
                if oldSize == len(conformedNames):
                    break
                else:
                    oldSize = len(conformedNames)
